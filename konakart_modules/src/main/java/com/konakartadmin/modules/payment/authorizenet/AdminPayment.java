...
import com.konakartadmin.modules.AdminBasePayment;
import com.konakartadmin.modules.AdminPaymentIf;
...
public class AdminPayment extends AdminBasePayment implements AdminPaymentIf
{
    ...
    public NameValue[] execute(PaymentOptions options) throws Exception
    {
         ...
         * subscriptions
         */
        if (options.getAction() == 1)
        {
            return createSubscription(options);
        } else if (options.getAction() == 2)
        {
            return updateSubscription(options);
        } else if (options.getAction() == 3)
        {
        ...
        } else if (mode == 3)
        {
            ...
            if (authTransId == null)
            {
                throw new KKAdminException(
                        "The suthorized transaction cannot be confirmed since a transaction id cannot be found on the order");
            }
        ...
        if (gatewayResult != null && gatewayResult.equals(approved))
        {
            ...
            getAdminOrderMgr().updateOrderStatus(order.getId(),
     ...
     * @throws TorqueException
     */
    public void setStaticVariables() throws TorqueException, DataSetException, Exception
    {
        ...
        staticData.setTestMode(getAdminConfigMgr().getConfigurationValueAsBool(
                MODULE_PAYMENT_AUTHORIZENET_TESTMODE, true));
        staticData.setZone(getAdminConfigMgr().getConfigurationIntValueOrDefault(
                MODULE_PAYMENT_AUTHORIZENET_ZONE, 0));
        staticData.setSortOrder(getAdminConfigMgr().getConfigurationIntValueOrDefault(
                MODULE_PAYMENT_AUTHORIZENET_SORT_ORDER, 0));
        staticData.setShowCVV(getAdminConfigMgr().getConfigurationValueAsBool(
    ...
    /**
     * Method that needs to be implemented to create a subscription
     * 
     * @param options
     * @return Returns an array of NameValue objects that may contain any return information
     *         considered useful by the caller.
    ...
    protected NameValue[] createSubscription(PaymentOptions options) throws Exception
    {
        ...
        AdminSubscription subscription = getAdminBillingMgr().getSubscription(
        ...
        /*
         * Insert code to actually send request to AuthorizeNet
         */
