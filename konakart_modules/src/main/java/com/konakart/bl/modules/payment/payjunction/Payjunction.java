...
public class Payjunction extends BasePaymentModule implements PaymentInterface
{
    ...
    /**
     * Used to enable or disable Debug Mode
     */
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_USERNAME);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_PASSWORD);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_SECURITY);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_DEBUG_MODE);
        if (conf == null)
        {
            staticData.setDebugMode(true);
        } else
        {
            if (conf.getValue().trim().equalsIgnoreCase("false"))
            {
                staticData.setDebugMode(false);
            } else
            {
                staticData.setDebugMode(true);
            }
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYJUNCTION_SORT_ORDER);
        if (conf == null)
        {
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_PAYJUNCTION_STATUS);
    }
