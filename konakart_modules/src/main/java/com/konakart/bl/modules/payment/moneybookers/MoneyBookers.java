...
public class MoneyBookers extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_DECLINE_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_ACCEPT_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_REQUEST_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_PAY_TO_EMAIL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_MONEYBOOKERS_ORDER_STATUS_ID);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        if (bName != null)
        {
            ...
            if (len >= 2)
            {
                ...
                parmList.add(new NameValue("firstname", firstName.toString()));
                parmList.add(new NameValue("lastname", names[len - 1]));
            }
        }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_MONEYBOOKERS_STATUS);
    }
    ...
    protected class StaticData
    {
        ...
        // is
        // set
        // as a product parameter in the setup section of the MoneyBookers web site (access_url)
