...
public class CyberSource extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_REQUEST_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_RESPONSE_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_MERCHANT_ACC);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_SHARED_SECRET);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_SERIAL_NUMBER);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_VERSION);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_CYBERSOURCE_SORT_ORDER);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        // Put the shared secret on custom1 for security - we'll remove it asap
        ...
        parmList.add(new NameValue(CYBERSOURCE_RESPONSE_RECEIPT_URL, sd.getResponseUrl().replaceFirst(
                hostPortSubstitute, info.getHostAndPort())));
        parmList.add(new NameValue(CYBERSOURCE_RESPONSE_DECLINE_URL, sd.getResponseUrl().replaceFirst(
                hostPortSubstitute, info.getHostAndPort())));
        ...
        if (bName != null)
        {
            ...
            parmList.add(new NameValue(CYBERSOURCE_BILLTO_FNAME, names[0]));
            parmList.add(new NameValue(CYBERSOURCE_BILLTO_LNAME, names[1]));
        }
        ...
        // Set the delivery name. If the name field consists of more than two strings, we take the
        // last one as being the surname.
        String dName = order.getDeliveryName();
        if (dName != null)
        {
            String[] names = splitNameIntoFirstAndLastNames(dName);
            parmList.add(new NameValue(CYBERSOURCE_SHIPTO_FNAME, names[0]));
            parmList.add(new NameValue(CYBERSOURCE_SHIPTO_LNAME, names[1]));
        }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_CYBERSOURCE_STATUS);
    }
