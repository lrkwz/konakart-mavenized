...
public class BarclaycardSmartPayApi extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_REQUEST_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_3D_STATUS);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_3D_RESPONSE_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_MERCHANT_ACC);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_USER_ID);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_PASSWORD);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_BC_SPAY_API_ORDER_STATUS_ID);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        if (bName != null)
        {
            ...
            if (len >= 2)
            {
                ...
                parmList.add(new NameValue("firstname", firstNameStr));
                String lastNameStr = names[len - 1];
                parmList.add(new NameValue("lastname", lastNameStr));

                String custReference = order.getCustomerId() + "~" + firstNameStr;
                if (lastNameStr != null)
                {
                    custReference += " " + lastNameStr;
                }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_BC_SPAY_API_STATUS);
    }
