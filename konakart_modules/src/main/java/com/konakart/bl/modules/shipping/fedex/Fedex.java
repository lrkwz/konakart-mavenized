...
public class Fedex extends BaseShippingModule implements ShippingInterface
{
    ...
    {
        return isAvailable(getEng(), MODULE_SHIPPING_FEDEX_STATUS);
    }
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_TAX_CLASS);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_PASSWORD);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_ACCOUNT_NUMBER);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_METER_NUMBER);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_KEY);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_MEASUREMENT_UNIT);
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_HANDLING);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_SERVICE_TYPE);
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_END_POINT_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_DROP_OFF_TYPE);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_RATE_REQUEST_TYPE);
        if (conf == null)
        {
            throw new KKException(
    ...
    {
        StaticData sd = staticDataHM.get(getStoreId());
        KKConfiguration conf = getEng().getConfiguration(MODULE_SHIPPING_FEDEX_PACKAGING_TYPE);
        if (conf != null)
        {
            ...
            for (int i = 0; i < weightPackagesArray.length; i++)
            {
                ...
                if (weightPackageArray.length == 2)
                {
                    ...
                    WeightPackage wp = new WeightPackage(new BigDecimal(weightPackageArray[0]), pt);
    ...
    private void setExcludeServiceMap() throws KKException
    {
        ...
        KKConfiguration conf = getEng().getConfiguration(
