...
public class WorldPayXMLRedirect extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        String reqUrl = getEng().getConfigurationValue(MODULE_PAYMENT_WP_XML_REDIRECT_REQUEST_URL);
        ...
        String merchantCode = getEng().getConfigurationValue(
        ...
        String password = getEng().getConfigurationValue(
        ...
        String responseUrl = getEng().getConfigurationValue(
        ...
        String sharedSecret = getEng().getConfigurationValue(
        ...
        KKConfiguration conf = getEng().getConfiguration(MODULE_PAYMENT_WP_XML_REDIRECT_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WP_XML_REDIRECT_SORT_ORDER);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        // Set the shipping name. If the name field consists of more than two strings, we take the
        // last one as being the surname.
        String bName = order.getDeliveryName();
        if (bName != null)
        {
            ...
            if (len >= 2)
            {
                ...
                parmList.add(new NameValue("firstName", URLEncoder.encode(firstName.toString(),
                        "UTF-8")));
                parmList.add(new NameValue("lastName", URLEncoder.encode(names[len - 1], "UTF-8")));
            }
        }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_WP_XML_REDIRECT_STATUS);
    }
