...
public class ShippingMgr extends BaseMgr implements ShippingMgrIf
{
    ...
    public void refreshConfigs() throws Exception
    {
        synchronized (mutex)
        {
            ...
            int originCountry = mgr.getConfigurationValueAsInt(SHIPPING_ORIGIN_COUNTRY);
            if (originCountry != KKConstants.NOT_SET)
            {
                sd.setShippingOriginCountry(getTaxMgr().getCountryPerId(originCountry));
            }

            String originZip = mgr.getConfigurationValue(SHIPPING_ORIGIN_ZIP);
            if (originZip != null)
            {
                sd.setShippingOriginZip(originZip);
            }

            sd.setShippingMaxWeight(mgr.getConfigurationValueAsBigDecimal(SHIPPING_MAX_WEIGHT,
                    new BigDecimal(1000000)));
            sd.setShippingBoxWeight(mgr.getConfigurationValueAsBigDecimal(SHIPPING_BOX_WEIGHT,
                    new BigDecimal(0)));
            sd.setShippingBoxPadding(mgr.getConfigurationValueAsBigDecimal(SHIPPING_BOX_PADDING,
                    new BigDecimal(0)));
            ...
            String modsInstalled = mgr.getConfigurationValue(MODULE_SHIPPING_INSTALLED);
            ...
            sd.setFreeShipping(mgr.getConfigurationValueAsBool(FREE_SHIPPING, false));

            sd.setFreeShippingOver(mgr.getConfigurationValueAsBigDecimal(FREE_SHIPPING_OVER,
                    new BigDecimal(0)));

            String shipDestination = mgr.getConfigurationValue(SHIPPING_DESTINATION);
            if (shipDestination != null)
            {
                sd.setShippingDestination(shipDestination);
            }

            int storeCountryInt = mgr.getConfigurationValueAsInt(STORE_COUNTRY);
