...
public class BaseOrderTotalModule extends BaseModule
{
    ...
    protected int getCustomInt(String customAttr, int customId) throws KKException
    {
        ...
        {
            throw new KKException("Custom" + customId + " must be set to a valid integer.");
        }
    }
    ...
    protected BigDecimal getCustomBigDecimal(String customAttr, int customId) throws KKException
    {
        ...
        {
            throw new KKException("Custom" + customId + " must be set to a valid decimal number.");
        }
    }
    ...
    protected boolean getCustomBoolean(String customAttr, int customId) throws KKException
    {
        try
        {
            ...
            {
                throw new KKException("Custom" + customId + " must be set to true or false.");
            }
        } catch (Exception e)
        {
            throw new KKException("Custom" + customId + " must be set to true or false.");
     ...
     * @throws Exception
     */
    protected OrderTotal getDiscountOrderTotalFromList(Order order, List<OrderTotal> orderTotalList)
            throws Exception
    {

        if (orderTotalList == null || order == null)
        {
            return null;
        }

        if (orderTotalList.size() == 1)
        {
            // Simple case with just one applicable order total
            OrderTotal localOt = orderTotalList.get(0);
            ...
            {
                setCouponIds(order, Integer
                        .toString(localOt.getPromotions()[0].getCoupon().getId()));
            }
            return localOt;
        } else if (orderTotalList.size() > 1)
        {
            ...
            for (Iterator<OrderTotal> iter = orderTotalList.iterator(); iter.hasNext();)
            {
            ...
            if (cumulativeOT != null)
            {
                ...
                orderTotalList.add(cumulativeOT);
            }

            // Select biggest order total object
            OrderTotal selectedOT = orderTotalList.get(0);
            for (Iterator<OrderTotal> iter = orderTotalList.iterator(); iter.hasNext();)
            {
                OrderTotal localOt = iter.next();
