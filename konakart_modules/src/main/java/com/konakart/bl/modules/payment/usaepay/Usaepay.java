...
public class Usaepay extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_KEY);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_REQUEST_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_ORDER_STATUS_ID);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_TESTMODE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_SHOW_ADDRESS);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_SHOW_POSTCODE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_USAEPAY_SHOW_CVV);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        if (bName != null)
        {
            ...
            if (len >= 2)
            {
                ...
                parmList.add(new NameValue("UMbillfname", firstName.toString()));
                parmList.add(new NameValue("UMbilllname", names[len - 1]));
            }
        }
        ...
        parmList.add(new NameValue("UMbillstate", order.getBillingState()));
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_USAEPAY_STATUS);
    }
