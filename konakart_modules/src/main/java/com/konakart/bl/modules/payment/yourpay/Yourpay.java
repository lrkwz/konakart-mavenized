...
public class Yourpay extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_STORENAME);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_REFERRER_URL);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_ORDER_STATUS_ID);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_ADD_CCV);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_TEST_MODE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_YOURPAY_DEBUG_MODE);
        if (conf == null)
        {
            staticData.setYourPayDebugMode(true);
        } else
        {
            if (conf.getValue().equalsIgnoreCase("false"))
            {
                staticData.setYourPayDebugMode(false);
            } else
            {
                staticData.setYourPayDebugMode(true);
            }
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        // Only enable debug when testing...
        if (sd.isYourPayDebugMode())
        {
            parmList.add(new NameValue("debug", "true"));
        }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_YOURPAY_STATUS);
    }
    ...
    protected class StaticData
    {
        ...
        private boolean yourPayDebugMode;
        ...
        /**
         * @return the yourPayDebugMode
         */
        public boolean isYourPayDebugMode()
        {
            return yourPayDebugMode;
        }

        /**
         * @param yourPayDebugMode
         *            the yourPayDebugMode to set
         */
        public void setYourPayDebugMode(boolean yourPayDebugMode)
        {
            this.yourPayDebugMode = yourPayDebugMode;
        }
