...
public class Worldpay extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WORLDPAY_REQUEST_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WORLDPAY_INST_ID);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WORLDPAY_TEST_MODE);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WORLDPAY_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_WORLDPAY_SORT_ORDER);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        parmList.add(new NameValue("M_cs1", getEng().getSecretKeyForOrderId(order.getId())));
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_WORLDPAY_STATUS);
    }
