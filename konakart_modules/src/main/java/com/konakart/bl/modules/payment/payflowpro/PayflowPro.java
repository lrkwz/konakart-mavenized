...
public class PayflowPro extends BasePaymentModule implements PaymentInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_REQUEST_URL);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_LOGIN);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_VENDOR);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_PARTNER);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_PWD);
        if (conf == null)
        {
            throw new KKException(
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_ZONE);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_SORT_ORDER);
        if (conf == null)
        {
        ...
        conf = getEng().getConfiguration(MODULE_PAYMENT_PAYFLOWPRO_SHOW_CVV);
        if (conf == null)
        {
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        if (bName != null)
        {
            ...
            if (len >= 2)
            {
                ...
                parmList.add(new NameValue("FIRSTNAME", Utils.trim(firstName.toString(), 25)));
                parmList.add(new NameValue("LASTNAME", Utils.trim(names[len - 1], 25)));
            }
        }
    ...
    {
        return isAvailable(getEng(), MODULE_PAYMENT_PAYFLOWPRO_STATUS);
    }
