...
import com.konakart.bl.modules.payment.cybersourcehop.CyberSourceHOP;
import com.konakart.bl.modules.payment.cybersourcehop.CyberSourceHOPHMACTools;
 ...
 */
public class CyberSourceHOPResponseAction extends BaseGatewayAction
{
    /**
     * The <code>Log</code> instance for this application.
     */
    protected Log log = LogFactory.getLog(CyberSourceHOPResponseAction.class);

    // Configuration constants
    private static final String MODULE_PAYMENT_CYBERSOURCEHOP_STATUS = "MODULE_PAYMENT_CYBERSOURCEHOP_STATUS";
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
                    ...
                    if (paramName != null)
                    {
                        ...
                        } else if (paramName.equalsIgnoreCase("reconciliationID"))
                        {
                            reconciliationID = paramValue;
                                ...
                                .equalsIgnoreCase(CyberSourceHOP.CYBERSOURCEHOP_MERCHANT_REF))
                        {
            ...
            if (log.isDebugEnabled())
            {
                log.debug(CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE + " Raw Response data:\n"
                        + fullGatewayResponse);
                log.debug("\n    decision                      = " + decision
            ...
            boolean enabled = kkAppEng.getConfigAsBoolean(MODULE_PAYMENT_CYBERSOURCEHOP_STATUS,
                    false);
            if (!enabled)
            {
                String msg = "The " + CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE
                        + " module isn't enabled but we just " + "received a callback message";
                saveIPNrecord(kkAppEng, orderId, CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE,
            ...
            if (decision == null)
            {
                String msg = "No decision returned for the "
                        + CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE + " module";
                saveIPNrecord(kkAppEng, orderId, CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE,
                        fullGatewayResponse, decision, reconciliationID, RET1_DESC + msg, RET1);
                throw new Exception(msg);
            }
            ...
            Boolean validateSignature = CyberSourceHOPHMACTools.verifyBase64EncodedSignature(
                    kkAppEng.getConfig(CyberSourceHOP.MODULE_PAYMENT_CYBERSOURCEHOP_SHARED_SECRET),
            ...
            if (!validateSignature)
            {
                String msg = "Signature Validation Failed for the "
                        + CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE + " module - orderId "
                        + orderId;
                saveIPNrecord(kkAppEng, orderId, CyberSourceHOP.CYBERSOURCEHOP_GATEWAY_CODE,
                        fullGatewayResponse, decision, reconciliationID, RET1_DESC + msg, RET1);
                throw new Exception(msg);
            ...
            if (!decision.equals("ACCEPT"))
            {
                ...
                return new ActionForward(retUrl.toString(), true);
            }
