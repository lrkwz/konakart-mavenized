...
public class MoneyBookersAction extends BaseGatewayAction
{
    ...
    // Configuration constants for MoneyBookers
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            kkEngine = kkAppEng.getEng();

            // We get from configurations, the username and password used to log into the engine
            // in order to save the changes of the IPN
            username = kkAppEng.getConfig(MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_USERNAME);
            password = kkAppEng.getConfig(MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_PASSWORD);
            secretWord = kkAppEng.getConfig(MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_SECRET_WORD);

            if (username == null || password == null || secretWord == null)
            {
                throw new Exception(
                        "The callback username and password must be defined for the MoneyBookers module by"
                                + " setting the configuration variables MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_USERNAME"
                                + " and MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_PASSWORD and MODULE_PAYMENT_MONEYBOOKERS_CALLBACK_SECRET_WORD");
            }
