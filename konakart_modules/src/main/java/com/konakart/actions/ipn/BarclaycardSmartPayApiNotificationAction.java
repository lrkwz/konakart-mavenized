...
public class BarclaycardSmartPayApiNotificationAction extends BaseGatewayAction
{
    ...
    // Configuration constants for BarclaycardSmartPayApi
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            } catch (Exception e)
            {
                ...
                {
                    log.warn("Error parsing merchantReference: '" + merchantReference + "'");
                }
                e.printStackTrace();
            }
            ...
            boolean enabled = kkEngine.getConfigurationValueAsBool(
                    MODULE_PAYMENT_BC_SPAY_API_STATUS, false);
            if (!enabled)
            {
            ...
            username = kkEngine.getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_CALLBACK_USERNAME);
            password = kkEngine.getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_CALLBACK_PASSWORD);
            ...
            if (username == null || password == null)
            {
                throw new Exception(
                        "The callback username and password must be defined for the "
                                + BarclaycardSmartPayApi.BC_SPAY_API_GATEWAY_CODE
                                + " module by"
                                + " setting the configuration variables MODULE_PAYMENT_BC_SPAY_API_CALLBACK_USERNAME"
                                + " and MODULE_PAYMENT_BC_SPAY_API_CALLBACK_PASSWORD");
            }

            hmacKey = kkEngine.getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_HMAC_KEY);
            ...
            httpAuthStr = kkEngine.getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_HTTP_AUTH);
            if (httpAuthStr != null && Boolean.valueOf(httpAuthStr))
            {
                ...
                httpUsername = kkEngine
                        .getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_HTTP_USERNAME);
                httpPassword = kkEngine
                        .getConfigurationValue(MODULE_PAYMENT_BC_SPAY_API_HTTP_PASSWORD);
