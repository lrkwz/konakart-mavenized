...
public class WorldPayAction extends BaseGatewayAction
{
    ...
    // Configuaration constants for WorldPay
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            username = kkAppEng.getConfig(MODULE_PAYMENT_WORLDPAY_CALLBACK_USERNAME);
            password = kkAppEng.getConfig(MODULE_PAYMENT_WORLDPAY_CALLBACK_PASSWORD);

            if (username == null || password == null)
            {
                throw new Exception(
                        "The callback username and password must be defined for the WorldPay module by"
                                + " setting the configuration variables MODULE_PAYMENT_WORLDPAY_CALLBACK_USERNAME"
                                + " and MODULE_PAYMENT_WORLDPAY_CALLBACK_PASSWORD");
            }
            ...
            if (request != null)
            {
                ...
                while (en.hasMoreElements())
                {
                    ...
                    if (paramName != null)
                    {
                        if (WorldPayAction.SECRET_KEY_PARAMETER.equalsIgnoreCase(paramName))
                        {
                            secretKey = paramValue;
                        } else if (WorldPayAction.TRANSACTION_STATUS_PARAMETER
                                .equalsIgnoreCase(paramName))
                        {
                            transactionType = paramValue;
                        } else if (WorldPayAction.TRANSACTION_ID_PARAMETER
                                .equalsIgnoreCase(paramName))
                        {
                        ...
                        } else if (WorldPayAction.ORDER_ID_PARAMETER.equalsIgnoreCase(paramName))
                        {
                            ...
                            } catch (Exception e)
                            {
                                ...
                                {
                                    log
                                            .info("WorldPay callback. Parameter M_cs2 should be the order id but is not a number : "
                                                    + paramValue);
                                }
                            }
                        }
                ...
                /*
                 * Get the order associated with the secret key. The secret key was sent to WorldPay
                 * during the original post in the "custom" parameter. It is associated to an order
                 * and is a safe way of determining that the call back from WorldPay is genuine as
                ...
                /*
                 * Flag an error if the order id from the secret key doesn't match the order id
                 * received in a callback parameter.
                 */
                if (orderId != orderIdFromGateway)
                {
