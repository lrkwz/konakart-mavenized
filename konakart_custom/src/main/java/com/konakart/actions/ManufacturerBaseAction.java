...
public class ManufacturerBaseAction extends BaseAction
{
            ...
            throws KKAppException, KKException
    {
        ...
        // Set page title
        String title = getCatMessage(request, "seo.manufacturer.title.template");
        title = title.replace("$manufacturer", manu.getName());
        kkAppEng.setPageTitle(title);

        // Set the meta description
        String description = getCatMessage(request, "seo.manufacturer.meta.description.template");
        ...
        // Set bread crumbs
        String url = "/ShowProductsForManufacturer.do?manuId=";
        kkAppEng.nav.set(manu.getName(), url + manu.getId());
            ...
            HttpServletResponse response, int seoFormat, int code)
    {
        ...
        } else if (seoFormat == SEO_PARAMETERS)
        {
            ...
            if (manu.getName() != null && manu.getName().length() > 0
                    && getCatMessage(request, "seo.manufacturer.title.template").length() > 0)
            {
                try
                {
                    url.append("&");
