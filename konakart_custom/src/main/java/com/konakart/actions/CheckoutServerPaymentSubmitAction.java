...
import com.konakart.forms.CreditCardForm;
...
public class CheckoutServerPaymentSubmitAction extends BaseAction
{
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        try
        {
            ...
            pd.setCcCVV(ccForm.getCvv());
            pd.setCcExpiryMonth(ccForm.getExpiryMonth());
            pd.setCcExpiryYear(ccForm.getExpiryYear());
            pd.setCcNumber(ccForm.getNumber());
            pd.setCcOwner(ccForm.getOwner());
            pd.setCcPostcode(ccForm.getPostcode());
            pd.setCcStreetAddress(ccForm.getStreetAddress());
            pd.setCcType(ccForm.getType());

            /*
             * In the Struts config file you must define a forward with the name of the code
             */
            return mapping.findForward(order.getPaymentDetails().getCode());
