 ...
 */
public class DeleteAddrSubmitAction extends BaseAction
{
            ...
            HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            {
                kkAppEng.getCustomerMgr().deleteAddressFromCustomer(
                        kkAppEng.getCustomerMgr().getSelectedAddr().getId());
            } else
            {
                throw new KKAppException("An address to delete has not been selected");
            }

            // Add a message to say all OK
            ActionMessages msgs = new ActionMessages();
            msgs.add(ActionMessages.GLOBAL_MESSAGE,
                    new ActionMessage("address.book.body.removedok"));
